	...
	factory: (require) => {
		...
		*/
		function deriveReadout(node, model, t, options = {}) {
			...
			const completedAt = safeNumber(node.timing?.completedTime);
			...
			return readoutSegments({
				...
				tokensPerSecond: tokensPerSecond(outputTokens, decodeMsOf(node.timing)),
				...
				ttftMs: node.timing === null || typeof node.timing !== "object" ? null : elapsedMs(node.timing.stepStartTime, node.timing.firstTokenTime)
			}, t);
		}
		/**
		* Conversation definition for steps that ended WITHOUT a finalized
		* visible text answer — a manual stop, a failure, or a hard abort. Such
		* steps never mount the assistant action strip, and a stop before the
		* first token does not even close the turn (no `turn/end`), so no
		* existing seat can host their readout. This definition materializes
		* its own chat node per such step, carrying the turn/step coordinates,
		* the reasoning text accumulated before the interruption, and the
		* token timing facts for a throughput reading; the model name still
		* comes from the `dsh-response-meta` host projection.
		*
		...
		*/
		...
		const abortedStepDefinition = {
			...
			match: (event) => {
				...
				if ((event.type === "assistant/chunk" || event.type === "assistant/message" || event.type === "llm/retry" || event.type === "step/end") && Number.isSafeInteger(turn) && Number.isSafeInteger(step)) return {
					id: abortedStepId(turn, step),
					role: "update"
				};
				return null;
			...
			update: (context, match) => {
				...
				if (event.type === "assistant/message") {
					...
					const usageRecord = usage === null || typeof usage !== "object" ? null : {
					...
					if (next.textFinalized === context.state.textFinalized && next.usage === context.state.usage && next.messageTime === context.state.messageTime) return context.state;
					return next;
				}
				...
				if (event.type === "assistant/chunk") {
					...
					const reasoning = chunk !== null && typeof chunk === "object" && chunk.type === "reasoning-delta" && typeof chunk.text === "string" ? chunk.text.length : 0;
					const text = chunk !== null && typeof chunk === "object" && chunk.type === "text-delta" && typeof chunk.text === "string" ? chunk.text.length : 0;
					const token = isTokenChunk(chunk);
					if (reasoning === 0 && text === 0 && !token) return context.state;
					const time = typeof event.time === "number" ? event.time : null;
					return {
						...context.state,
						reasoningChars: context.state.reasoningChars + reasoning,
						totalChars: context.state.totalChars + reasoning + text,
						firstTokenTime: token && context.state.firstTokenTime === null ? time : context.state.firstTokenTime,
						lastTokenTime: token && time !== null ? time : context.state.lastTokenTime
					};
				}
			...
			buildViewNode: (context) => {
				...
				if (state === undefined || state.ended !== true || state.textFinalized === true) return null;
		...
		/**
		* Assemble the readout line for one aborted step: model name, the
		* reasoning extent accumulated before the interruption, and a
		...
		*/
		...
		const css = ".drm_7cRfq_readout{color:var(--dsw-alias-label-tertiary);white-space:nowrap;font-size:11px;line-height:16px;font-variant-numeric:tabular-nums;user-select:none;pointer-events:none;margin-left:2px;display:inline-block;align-self:center}.drm_7cRfq_readout[data-dsh-response-meta=complete]{order:2;flex-basis:100%;white-space:normal;margin:2px 0 0;overflow-wrap:anywhere}.drm_7cRfq_abortedRow{padding:0 0 8px;display:flex}[data-time-hover-root] [data-slot$=assistant-actions]:has(>[data-dsh-response-meta=complete])~span:last-child{display:none}[data-time-hover-root]:has([data-dsh-response-meta=complete])>div:has(>[data-slot$=assistant-actions]){height:auto;flex-wrap:wrap;row-gap:4px}";
		...
		* session kit (useSession, useProjection, t).
		* @returns the readout span, or null when nothing is available.
		*/
		const ResponseMetaTag = (0, react.memo)(function ResponseMetaTag({ messageId, useSession, useProjection, t }) {
			...
			const turnTiming = useSession((snapshot) => {
			...
			const line = (0, react.useMemo)(() => {
				...
				const model = turnRecord === null || typeof turnRecord !== "object" ? void 0 : turnRecord.model;
				return deriveReadout(node, model, t, { turnTiming, now: Date.now() });
			}, [node, turnTiming, projection, t]);
		...
		/**
		* Readout row for one aborted step (ended without a finalized answer):
		* model name plus the partial reasoning extent accumulated before the
		* interruption. No usage record exists for those steps, so toks/s stays
		* absent. Renders nothing when there is nothing to say.
		...
		*/
		const ResponseMetaAbortedNode = (0, react.memo)(function ResponseMetaAbortedNode({ node, useProjection, t }) {
			...
			return (0, react_jsx_runtime.jsx)("div", {
				...
				"data-dsh-response-meta": "aborted",
		...
		* action-strip readout, and the aborted-step chat node. The latter
		* materializes for steps that ended without a final message — including
		* a manual stop before the first token, which does not even close the
		* turn — so every interrupted output still shows its model.
		...
		*/
		function apply(ctx) {
			...
			ctx.conversationEvents.register(abortedStepDefinition);
